...
`mac/com.ymac.clipshare.plist` の実行ファイルパスは現在、次の配置を前提としています。

```text
/Users/ymac/develop/develop-tools/clipshare/mac/.build/release/clipshare-mac
```

リポジトリを別の場所へ移動した場合は、plist内の `ProgramArguments` も実際の絶対パスに変更してください。
...
Tailscaleを使用する場合は、`ymac` の代わりにMacのTailscale MagicDNS名またはTailscale IPアドレスも指定できます。
...
2. Androidのホストが `ymac`、Tailscale MagicDNS名、または到達可能なIPアドレスになっている
